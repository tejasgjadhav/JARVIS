...
def build_pdf(d: dict, a: dict, narrative=None) -> bytes:
    ...
    story.append(Paragraph(
        ...
        "Automated research, not personalised investment advice. Personal research tool, not for distribution.</i>",
...
def build_pdf_short(d: dict, a: dict, narrative) -> bytes:
    ...
    _jev_pdf(story, a, body)
    story.append(Spacer(1, 10))
    story.append(Paragraph("<i>Data: Yahoo Finance (live). Technical note, not personalised investment advice. Personal research tool, not for distribution.</i>",
                 ParagraphStyle("disc", parent=body, fontSize=7.5, textColor=colors.HexColor("#" + GREY))))
    doc.build(story)
    return buf.getvalue()
